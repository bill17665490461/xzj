...
//获取用户名称    $uid = int()
function getMemberName($uid = 0){
    if (empty($uid)) {
        return false;
    }
    $getMember = Db::name('member')->where('id',$uid)->field('nickname')->find();
...
function area($id = 0){
    ...
    $list=Db::name('area')->where("id",$id)->find();
    return $list['name'];
...
//获取项目类型
function getCateType($cate_id){

    if (empty($cate_id)) {
        return false;
    }
    $list=Db::name('catetype')->where("id",$cate_id)->field('title')->find();
    return $list['title'];
}