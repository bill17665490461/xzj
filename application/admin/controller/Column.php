...
class Column extends Common
{
    ...
    {
        $clist = model('Column')->getColumn('id,path,type,title,name,ext_model_id,model_id,cover_picture,url,status,orders,navname');
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            switch ($data['type']) {
                //单页
                case 1:
                    $fields = ['type', 'path', 'model_id', 'ext_model_id', 'ext_model_id', 'title', 'name', 'meta_title', 'meta_keywords', 'meta_description', 'cover_picture', 'template_list', 'orders', 'status','navname'];
                    $scene = 'page';
                    break;
                //列表
                case 2:
                    ...
                    $fields = ['type', 'path', 'model_id', 'ext_model_id', 'title', 'name', 'meta_title', 'meta_keywords', 'meta_description', 'cover_picture', 'template_list', 'template_content', 'list_row', 'orders', 'status', 'url', 'listorder','navname'];
                    $scene = 'list';
                    break;
                //链接
                case 3:
                    $fields = ['type', 'path', 'title', 'name', 'cover_picture', 'url', 'orders', 'status','navname'];
                    $scene = 'link';
                    break;
                default:
            ...
            if (in_array($data['type'], [1, 2]) && isset($data['modelField']) && $data['ext_model_id']) {
                ...
                $data['modelField']['name'] = $data['navname'];
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $Column = model('Column');
            try {
                $Column->editColumn(['type', 'path', 'model_id', 'ext_model_id', 'title', 'name', 'meta_title', 'meta_keywords', 'meta_description', 'cover_picture', 'template_list', 'template_content', 'list_row', 'orders', 'status', 'url', 'listorder','navname'], $data, $info);
            } catch (\Exception $ex) {
                $this->error($ex->getMessage());
            }
            $message = '';
