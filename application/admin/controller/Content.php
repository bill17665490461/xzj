...
class Content extends Common
{
    ...
    public function add($cname = '')
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($data['category']) {
                $data['modelField']['category'] = $data['category'];
            }
            $data['modelFieldExt'] = isset($data['modelFieldExt']) ? $data['modelFieldExt'] : [];
            try {
            ...
            //获取园区类型
            $cateList = Db::name('catetype')->where('status = 1')->select();
            $this->assign([
                'placeList' => $placeList,
                'fieldList' => $fieldList,
                'province'  => $province,
    ...
    public function edit($cname = '', $id = 0,$tableName = '')
    {
        if (empty($cname)) {
            $this->error('参数错误~');
        }
        ...
        if ($this->request->isPost()) {
            ...
            if ($data['province']) {
                $data['modelField']['province'] = $data['province'];
            }
            if ($data['city']) {
                $data['modelField']['city'] = $data['city'];
            }
            
            if ($data['district']) {
                $data['modelField']['district'] = $data['district'];
            }
            
            if ($data['cate_id']) {
                $data['modelField']['cate_id'] = $data['cate_id'];
            }
            //dump($data);die;
            $ModelField = model('ModelField');
            $data['modelFieldExt'] = isset($data['modelFieldExt']) ? $data['modelFieldExt'] : [];
            ...
            if ($cname != 'fwcharacteristic' && $cname != 'fwobject') {
                $article=Db::name($tableName)->where("id",$contentId)->find();
            }
            ...
            if ($cname == 'government' OR $cname == 'investmentpark') {
                $getInfo = Db::name($cname)->where(['status'=>1,'id'=> $contentId])->field('cate_id')->find();
                if ($getInfo['cate_id'] > 0) {
                    $getCateInfo = Db::name('catetype')->where(['id'=>$getInfo['cate_id'],'status'=>1])->find();
                }
                $this->assign('cateInfo',$getCateInfo);
            }
            $this->assign([
                'placeList' => $placeList,
                'fieldList' => $fieldList,
