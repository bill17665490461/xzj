    ...
    public function deleteModelData($modeId, $ids)
    {
        ...
        if (is_array($ids)) {
            ...
            try {
                ...
                Db::name($modelInfo['table'])->where('id', 'in', $ids)->setField('status', 2);
                if (2 == $modelInfo['type']) {
                    Db::name($modelInfo['table'] . $this->ext_table)->where('did', 'in', $ids)->setField('status', 2);
            ...
            Db::startTrans();
            try {
                $d =Db::name($modelInfo['table'])->where('id',$ids)->setField('status', 2);
               
                if (2 == $modelInfo['type']) {

                    Db::name($modelInfo['table'] . $this->ext_table)->where('did', $ids)->setField('status', 2);
                }
                Db::commit();
            } catch (\Exception $e) {
                Db::rollback();
