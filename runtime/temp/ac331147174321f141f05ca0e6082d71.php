<?php /*a:2:{s:79:"E:\phpstudy\WWW\xzj\application\home\view\defaults\column\index\journalism.html";i:1541472189;s:62:"E:\phpstudy\WWW\xzj\application\home\view\defaults\layout.html";i:1541473421;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <!-- Standard Meta -->
     ...
     <div class="sitesearch">
      <div class="sitesearbox">
        ...
        <div class="hotsear">招商项目：
            <?php $sql_list=model('ModelField')->getDataList("catetype", "status=1 and create_time <1541473598", "*", "", "orders asc", "", null, 0, "");  $sum=count($sql_list);  foreach($sql_list as $key=>$vo): ?>
...
<!--面包屑end-->
          ...
          <?php if(is_array($list['data']) || $list['data'] instanceof \think\Collection || $list['data'] instanceof \think\Paginator): $i = 0; $__LIST__ = $list['data'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
		  	<div class="list_con">
		       <div class="list_tit">
		       <a target="_blank" href="/index.php/journalism/<?php echo htmlentities($vo['id']); ?>"><?php echo htmlentities($vo['title']); ?></a></div>
			   <div class="list_Introduction">　　<?php echo htmlentities($vo['abstract']); ?>
               [<a target="_blank" href="/index.php/journalism/<?php echo htmlentities($vo['id']); ?>">全文</a>]</div>
...
<div class="inspR_rank">
    ...
    <ul class="inspRlink">
    	<?php $sql_list=model('ModelField')->getDataList("journalism", "status=1 and create_time <1541473598", "id,cname,title,logo", "", "create_time desc", "12", null, 17, "");  $sum=count($sql_list);  foreach($sql_list as $key=>$vo): ?>
...
让招商引资更简单 更精准 更高效</p>
    ...
    <div class="fw_dt fl">
        ...
        <div class="fw_dt_info">
            <ul class="clearfix">
                <?php $sql_list=model('ModelField')->getDataList("article", "status=1 and create_time <1541473598", "id,cname,title", "", "create_time desc", "6", null, 36, "");  $sum=count($sql_list);  foreach($sql_list as $key=>$vo): ?>
